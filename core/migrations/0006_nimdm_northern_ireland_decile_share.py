"""
Deprivation (Northern Ireland) indicator for the NI Multiple Deprivation Measure.

NIMDM is rank-based with no published composite score, so only a decile-share metric
is modelled (never a synthesised score). Deprivation is per-nation and never merged
UK-wide, so this is its own NI-scoped code alongside the England IoD and Scotland SIMD
...
NAME = "NIMDM: share of SOAs in most-deprived national decile (Northern Ireland)"
...
class Migration(migrations.Migration):
    dependencies = [("core", "0005_simd_scotland_decile_share")]
